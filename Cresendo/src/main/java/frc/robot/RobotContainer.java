...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    joystick
        .x()
        .onTrue(s_Intakes.setTopIntakeState(Intakes.state.OUT))
        .onFalse(s_Intakes.setTopIntakeState(Intakes.state.OFF));
    joystick
        .b()
        .onTrue(s_Intakes.setTopIntakeState(Intakes.state.HP))
        .onFalse(s_Intakes.setTopIntakeState(Intakes.state.OFF));
    joystick
        .rightTrigger()
        .whileTrue(
            new RepeatCommand(
                s_Intakes.setBottomIntakeState(
                    Intakes.state.GROUND))); // Driver control to bring note in From Bottom
    // //.onFalse(s_Intakes.setTopIntakeState(Intakes.state.OFF));
    joystick
        .leftTrigger()
        .whileTrue(
            new RepeatCommand(
                s_Intakes.setBottomIntakeState(
                    Intakes.state.GROUNDOUT))); // Drive Control to spit note out Bottom
    // //.onFalse(s_Intakes.setTopIntakeState(Intakes.state.OFF));
    ...
    op.x().onTrue(s_Intakes.HPin());
    op.b().onTrue(s_Intakes.GroundPickUP());
    ...
    op.pov(0)
        .onTrue(
            new SequentialCommandGroup(
                s_Climb.unlockClimb(), s_Climb.goToClimberPosition(Positions.TOP)));
    op.pov(180)
        .onTrue(
            new SequentialCommandGroup(
                s_Climb.goToClimberPosition(Positions.BOTTOM), s_Climb.lockClimb()));
