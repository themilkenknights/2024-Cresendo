...
public class Intakes extends SubsystemBase {
  ...
  public Command TopIntakeByBeambreak(){
    return new SequentialCommandGroup(setTopIntakeState(state.FWD),waitUntil(this::getBackIR),setTopIntakeState(state.OFF));
}

public Command TopOutakeByBeambreak(){
    return new SequentialCommandGroup(setTopIntakeState(state.FWD),waitUntil(this::getNotFrontIR),waitSeconds(0.25),setTopIntakeState(state.OFF));
}
