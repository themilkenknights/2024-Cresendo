...
public class Intakes extends SubsystemBase {
  ...
  public Intakes(IntakeElevator elevator) {
    ...
    final ParallelCommandGroup defultCommandGroup =
        new ParallelCommandGroup(setBottomIntakeState(state.OFF), setTopIntakeState(state.OFF));
  ...
  public Command HPin() {
    return new SequentialCommandGroup(
            intakeElevator.gotoHeight(Positions.HP),
            TopIntakeByBeambreak(),
            intakeElevator.gotoHeight(Positions.GROUND))
        .withName("HP");
  }

  public Command TopIntakeByBeambreak() {
    return new SequentialCommandGroup(
        setTopIntakeState(state.HP),
        waitUntil(this::getFrontIR),
        waitSeconds(1), // TODO: tune time
        setTopIntakeState(state.OFF));
  }

  public Command TopOutakeByBeambreak() {
    return new SequentialCommandGroup(
        setTopIntakeState(state.OUT),
        waitUntil(this::getNotFrontIR),
        waitSeconds(0.25), // TODO: tune time
        setTopIntakeState(state.OFF));
  }

  public Command AmpOuttake() {
    return new SequentialCommandGroup(
            intakeElevator.gotoHeight(IntakeElevator.Positions.AMP), TopIntakeByBeambreak())
  ...
  public Command GroundPickUP() {
    return new SequentialCommandGroup(
            ...
            setBottomIntakeState(state.GROUND),
            setTopIntakeState(state.GROUND),
            waitUntil(this::getFrontIR),
            waitSeconds(1),
            setBottomIntakeState(state.OFF))
        .withName("GroundPickup");
  }
